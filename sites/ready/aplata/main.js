...
(function definition_page() {
	/* Функция выделения активной страницы.
	 *	 Считывает параметр data-page у ссылки и выделяет её если url содержит
	 *	 такое значение
	 */
	...
	(function definition() {
		// добавление класса подчеркивания и изменение цвета header
		links.forEach(link => {
			if (url.indexOf(link.dataset.page) != -1) {
				...
				if (link.dataset.page != 'index') {
					...
					body.style.backgroundColor = '#f5f7fc';
				} else {
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _libs_burger_menu_nav_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @libs/burger_menu/nav.js */ "./libs/burger_menu/nav.js");
/* harmony import */ var _libs_burger_menu_nav_js__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_libs_burger_menu_nav_js__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _libs_buttons_up_button_up_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @libs/buttons/up/button_up.js */ "./libs/buttons/up/button_up.js");
/* harmony import */ var _libs_buttons_up_button_up_js__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_libs_buttons_up_button_up_js__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _libs_forms_form_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @libs/forms/form.js */ "./libs/forms/form.js");
...
/* harmony import */ var _libs_responsive_responsive_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! @libs/responsive/responsive.js */ "./libs/responsive/responsive.js");
/* harmony import */ var _libs_responsive_responsive_js__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_libs_responsive_responsive_js__WEBPACK_IMPORTED_MODULE_4__);
...
// import '@libs/progressbar/progressbar.js';

// import '@libs/scroll/scroll.js';
// import '@libs/slider/slick.js';
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _inputmask_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./inputmask.js */ "./libs/forms/inputmask.js");
...
function form_processing() {
	...
	// обработка inputs & textarea
	inputs.forEach(input => {
		// обработка получения и потери фокуса
		on_focus(input);
		on_blur(input);
...
function find_options(options) {
	...
	[].forEach.call(options, function(option) {
		if (!option.children.length) {
			...
			list_options.push([option.text, 0, selected, value]);
		} else {
			const suboptions = option.children;
			list_options.push([option.getAttribute('label'), 1, false, '']);

			[].forEach.call(suboptions, function(suboption) {
				if (suboption.getAttribute('value')) {
					value = suboption.getAttribute('value');
				} else {
					value = '';
				}
				selected = suboption.getAttribute('selected') == 'selected';
				list_options.push([suboption.text, 0, selected, value]);
			});
		}
	});
...
function design_select(select) {
	// изменение настроек и стиля select
	...
	const list_options = find_options(select.children);

	if (!select.parentElement.classList.contains('form__select')) {
		select.replaceWith(div);
		div.style.cursor = 'pointer';
		div.classList.add('form__select', 'select');

		// обёртка с иконкой и заголовком
		div.append(head);
		head.classList.add('select__head');
		head.insertAdjacentHTML('beforeend', `<div class = "select__icon"></div>`);
		head.append(h5);
		h5.classList.add('select__title');
		...
		for (let i = 0; i < list_options.length; i++) {
			// заголовок
			if (list_options[i][2]) {
				h5.append(list_options[i][0]);
			}

			if (list_options[i][1] == 0) {
				ul.insertAdjacentHTML(
					'beforeend',
					`<li class = "select__item" data-value="${list_options[i][3]}">${list_options[i][0]}</li>`,
				);
			} else {
				ul.insertAdjacentHTML(
					'beforeend',
					`<li class = "select__subtitle">${list_options[i][0]}</li>`,
				);
			}
		}

		div.insertAdjacentHTML('beforeend', `<div class = "select__arrow"></div>`);
	}
}
...
function click_events() {
	...
	const select_title = document.querySelectorAll('.select__title');
	const select_items = document.querySelectorAll('.select__item');

	body.onclick = () => {
		if (form_selects.length > 0) {
			form_selects.forEach(select => {
				const img = select.lastElementChild;
				const list = select.children[1];
	...
	select_items.forEach(item => {
		item.onclick = event => {
			// длинное имя т.к. напрямую можно обратиться только ко всем заголовкам
			item.parentElement.previousElementSibling.lastElementChild.textContent = item.textContent;
		};
	});
}
...
(function responsive() {
	 ...
	 *
	 * 	v. 02 (14/03/2020)
	 * 	moving = (move_class, old_move_class, new_move_class, position, hide) - функция поодиночного перемещения
	 ...
	 * 		hide - скрывать родительский класс (old_move_class) для перемещаемого элемента
	 *
	 *
	 * 	v. 01 (14/03/2020)
	...
	const moving = (move_class, old_move_class, new_move_class, position='start', hide=true) => {
		...
		function transfer_element(move) {
			...
			if (screen_width < WIDTH) {
				...
				if (hide) {old_move.style.display = 'none'};
			} else {
	...
	moving('.move__1', '.old-move__1', '.new-move__1', 'end', true);
